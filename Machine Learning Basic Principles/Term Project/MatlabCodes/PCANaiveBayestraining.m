...
p1 = elected/test_size;
p0 = 1 - p1;


%covariance Sj^2 j is from 1 to d
Cov = cov(SCORE(1:test_size,:));
...
for i=test_size+1:n
    ...
    k1 = 0; k0 = 0;
    for j=1:d
       k1 = k1 + power((X(j) - M(1,j)),2)/Cov(j,j); 
       k0 = k0 + power((X(j) - M(2,j)),2)/Cov(j,j);
    end
    g1 = -0.5 * k1;
    g0 = -0.5 * k0;
    if g1 >= g0
   ...
   for i=1:test_size
       for r = 1:unique_parties
           if(strcmp(T_Value{i,2},party_names(r,1)) == 1)
                party_details(r,j+2) = party_details(r,j+2) +  SCORE(i,j); % summing up the values of a certain dimension for all the samples under the same party and storing them in party_details matrix
           end
       end  
   end
...
for i=test_size+1:n %iterate over each sample
    ...
    for j=1:d % iterate over each dimension
        for p = 1:unique_parties %iterate over each party
            dvalues(p,1) = dvalues(p,1) + power((X(j) - party_details(p,j+2)),2)/Cov(j,j); %calculate the value of the discriminator function
        end
    end
    
    for p = 1:unique_parties
            dvalues(p,1) = -0.5 * dvalues(p,1) + log(party_details(p,2)); %complete the calculation of the discriminator functions associated with each party
    end
...
fprintf('\n\n Here PCA and Naive Bayes Classifier (Class Specific Mean,Common covariance matrix) is  used. \n The evaluation is:\n F_elected = %1.5f \n F_not_elected = %1.5f \n F_party = %1.5f \n',F_elected,F_not_elected,F_party);
