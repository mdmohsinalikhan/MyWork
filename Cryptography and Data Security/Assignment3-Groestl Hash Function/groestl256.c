...
/*START: Preparing the initial vector */
	...
	/*printf("Here is the initial vector:\n");
	for(i=0;i<8;i++)
	{
		for(j=0;j<8;j++)
		{
			printf("%x ",initial_state[i][j]);
		}
...
/*START: Finding  the number of blocks */
	...
	//printf("The length of the padded message is:%ld and the number of blocks is: %ld\n",var1,no_of_blocks);
...
/*		printf("Here is the block content:\n");
			for(j=0;j<8;j++)
			{
				for(k=0;k<8;k++)
				{
					printf("%x ",block_content[j][k]);
				}
			printf("\n");
			}
*/
		...
		/*END: P */
			...
			/*printf("Here is the output block after processing the %dth block:\n",i);			
			for(j=0;j<8;j++)
			{
				for(k=0;k<8;k++)
				{
...
//printf("\nFunction P has been called");	
		...
		/*printf("Here is the initial round constant vecotr for P:\n");
		for(i=0;i<8;i++)
		{
			for(j=0;j<8;j++)
			{
				printf("%x ",round_constant[i][j]);
			}
		printf("\n");
		}*/
		/*END: Initializing the Round Constant*/
			...
			{	
				...
				//printf("Here is the block content after adding  the round constant P at %dth iteration:\n",i);
				addroundconstantp();
				//printf("Here is the block content after doing the subbyte at %dth iteration:\n",i);
				subbyte();
				//printf("Here is the block content after doing the shiftbytep at %dth iteration:\n",i);
				shiftbytep();
				//printf("Here is the block content after doing the mixbyte at %dth iteration:\n",i);
				mixbyte();
			}
...
//printf("\nFunction P has been called");	
		...
		/*END: Initializing the Round Constant*/
			...
			{	
				...
				//printf("Q:Here is the block content after adding  the round constant Q at %dth iteration:\n",i);
				addroundconstantq();
				//printf("Q:Here is the block content after doing the subbyte at %dth iteration:\n",i);
				subbyteq();
				//printf("Q:Here is the block content after doing the shiftbyteq at %dth iteration:\n",i);
				shiftbyteq();
				//printf("Q:Here is the block content after doing the mixbyte at %dth iteration:\n",i);
				mixbyteq();
			}
...
unsigned char temp,temp1,temp2,temp3,temp4,temp5,temp6;
			...
			/*START:Shifting the fifth row */
				//no change rquired as per the algorithm
			/*END:Shifting the fifth row */
...
temp_block_content[0][0]	= 	xtime(block_content[0][0]) ^
					xtime(block_content[1][0]) ^
					xtime(block_content[2][0])^block_content[2][0] ^
					xtime(xtime(block_content[3][0])) ^
					xtime(xtime(block_content[4][0]))^block_content[4][0] ^
					xtime(block_content[5][0])^block_content[5][0] ^
					xtime(xtime(block_content[6][0]))^block_content[6][0] ^
					xtime(xtime(block_content[7][0])^block_content[7][0])^block_content[7][0];

temp_block_content[1][0]	= 	xtime(xtime(block_content[0][0])^block_content[0][0])^block_content[0][0] ^
					xtime(block_content[1][0]) ^
					xtime(block_content[2][0]) ^
					xtime(block_content[3][0])^block_content[3][0] ^
					xtime(xtime(block_content[4][0])) ^
					xtime(xtime(block_content[5][0]))^block_content[5][0] ^
					xtime(block_content[6][0])^block_content[6][0] ^
					xtime(xtime(block_content[7][0]))^block_content[7][0];

temp_block_content[2][0]	= 	xtime(xtime(block_content[0][0]))^block_content[0][0] ^
					xtime(xtime(block_content[1][0])^block_content[1][0])^block_content[1][0] ^
					xtime(block_content[2][0]) ^
					xtime(block_content[3][0]) ^
					xtime(block_content[4][0])^block_content[4][0] ^
					xtime(xtime(block_content[5][0])) ^
					xtime(xtime(block_content[6][0]))^block_content[6][0] ^
					xtime(block_content[7][0])^block_content[7][0];


temp_block_content[3][0]	= 	xtime(block_content[0][0])^block_content[0][0] ^
					xtime(xtime(block_content[1][0]))^block_content[1][0] ^
					xtime(xtime(block_content[2][0])^block_content[2][0])^block_content[2][0] ^
					xtime(block_content[3][0]) ^
					xtime(block_content[4][0]) ^
					xtime(block_content[5][0])^block_content[5][0] ^
					xtime(xtime(block_content[6][0])) ^
					xtime(xtime(block_content[7][0]))^block_content[7][0];


temp_block_content[4][0]	= 	xtime(xtime(block_content[0][0]))^block_content[0][0] ^ 
					xtime(block_content[1][0])^block_content[1][0] ^
					xtime(xtime(block_content[2][0]))^block_content[2][0] ^
					xtime(xtime(block_content[3][0])^block_content[3][0])^block_content[3][0] ^
					xtime(block_content[4][0]) ^
					xtime(block_content[5][0]) ^
					xtime(block_content[6][0])^block_content[6][0] ^
					xtime(xtime(block_content[7][0]));

temp_block_content[5][0]	= 	xtime(xtime(block_content[0][0])) ^
					xtime(xtime(block_content[1][0]))^block_content[1][0] ^ 
					xtime(block_content[2][0])^block_content[2][0] ^
					xtime(xtime(block_content[3][0]))^block_content[3][0] ^
					xtime(xtime(block_content[4][0])^block_content[4][0])^block_content[4][0] ^
					xtime(block_content[5][0]) ^
					xtime(block_content[6][0]) ^
					xtime(block_content[7][0])^block_content[7][0];

temp_block_content[6][0]	= 	xtime(block_content[0][0])^block_content[0][0] ^
					xtime(xtime(block_content[1][0])) ^
					xtime(xtime(block_content[2][0]))^block_content[2][0] ^ 
					xtime(block_content[3][0])^block_content[3][0] ^
					xtime(xtime(block_content[4][0]))^block_content[4][0] ^
					xtime(xtime(block_content[5][0])^block_content[5][0])^block_content[5][0] ^
					xtime(block_content[6][0]) ^
					xtime(block_content[7][0]);


temp_block_content[7][0]	= 	xtime(block_content[0][0]) ^
					xtime(block_content[1][0])^block_content[1][0] ^
					xtime(xtime(block_content[2][0])) ^
					xtime(xtime(block_content[3][0]))^block_content[3][0] ^ 
					xtime(block_content[4][0])^block_content[4][0] ^
					xtime(xtime(block_content[5][0]))^block_content[5][0] ^
					xtime(xtime(block_content[6][0])^block_content[6][0])^block_content[6][0] ^
					xtime(block_content[7][0]);


block_content[0][0]=temp_block_content[0][0];
block_content[1][0]=temp_block_content[1][0];
block_content[2][0]=temp_block_content[2][0];
block_content[3][0]=temp_block_content[3][0];
block_content[4][0]=temp_block_content[4][0];
block_content[5][0]=temp_block_content[5][0];
block_content[6][0]=temp_block_content[6][0];
block_content[7][0]=temp_block_content[7][0];
*/
...
//printf("\n\n\nHere i am testing the nature of xtime: %x\n", xtime(xtime(0x02)^0x02)^0x02);
